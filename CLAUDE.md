...
service worker, the manifest, the icons and the persisted scroll position are
done. What M4 still needs is the iPhone itself, keyboard included — see ROADMAP
"Next action". `sync` and `relay` are still boundary docs only.
