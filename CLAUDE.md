...
`ui/` is now a working Svelte 5 app: identity, the cart, the list, the
ingredient library and settings, driven end to end by `ui-test` in headless
chromium. **What M4 still needs**: the recipe view and editor, the service
worker and manifest, and the iPhone itself — see ROADMAP "Next actions".
`sync` and `relay` are still boundary docs only.
...
| `lib/labels.ts` | Every French word in the app, and nothing else (0035) |
| `lib/format.ts` | Rendered number meets word: decimal comma, "≈", plurals, relative time |
